...
type VoucherSort = "newestUpdated" | "highestUses" | "highestDiscount" | "endingSoon"
...
function toInputDate(value?: string | null) {
  ...
  const date = new Date(value)
  return Number.isNaN(date.getTime()) ? getLocalDateTimeValue() : getLocalDateTimeValue(date)
}
...
function formatVoucherDiscount(voucher: Pick<AdminRestaurantVoucher, "type" | "discountValue">) {
...
function getInitialForm(restaurants: AdminRestaurantSummary[]): VoucherFormState {
  const now = new Date()
...
function getFormFromVoucher(voucher: AdminRestaurantVoucher): VoucherFormState {
  return {
    ...
    displayCarouselImageUrls: voucher.display?.carouselImageUrls?.join("\n") ?? "",
    ...
    code: voucher.code ?? "",
    mode: voucher.mode,
    type: voucher.type === "free-delivery" ? "free_delivery" : (voucher.type as VoucherFormType),
    ...
    discountValue: voucher.type === "free_delivery" ? "" : `${voucher.discountValue ?? ""}`,
    maxDiscountAmount: voucher.maxDiscountAmount ? `${voucher.maxDiscountAmount}` : "",
...
function toPayload(form: VoucherFormState, includeRestaurant: boolean): AdminVoucherPayload {
  const payload: AdminVoucherPayload = {
    fundedBy: form.fundedBy,
    scopeType: form.scopeType,
    selectedRestaurantIds:
      form.scopeType === "selected_restaurants" ? form.selectedRestaurantIds : [],
    audienceType: form.audienceType,
    selectedCustomerIds: form.audienceType === "selected_users" ? form.selectedCustomerIds : [],
    ...
    ownerSharePercent: form.fundedBy === "shared" ? Number(form.ownerSharePercent || 50) : undefined,
    platformSharePercent:
      form.fundedBy === "shared" ? 100 - Number(form.ownerSharePercent || 50) : undefined,
    ...
    discountValue: form.type === "free_delivery" ? 0 : Number(form.discountValue || 0),
    maxDiscountAmount: form.maxDiscountAmount.trim() ? Number(form.maxDiscountAmount) : 0,
  ...
  if (includeRestaurant && form.scopeType === "restaurant") payload.restaurantId = form.restaurantId
  return payload
}

function getScopeLabel(voucher: Pick<AdminRestaurantVoucher, "scopeType" | "selectedRestaurants" | "restaurant">) {
...
function VoucherFormSheet({
  ...
  const [form, setForm] = React.useState<VoucherFormState>(() => getInitialForm(restaurants))
  ...
  const targets: AdminPromotionTargets = targetsQuery.data ?? { categories: [], items: [] }
  ...
  const conflictPreview = React.useMemo(() => {
    ...
    return existingVouchers
      .filter((item) => item._id !== voucher?._id && !item.archivedAt && item.status === "Active")
      .filter((item) => {
        if (form.scopeType === "restaurant") return item.restaurantId === form.restaurantId
        if (form.scopeType === "all_restaurants") return item.scopeType === "all_restaurants"
        if (form.scopeType === "selected_restaurants") {
          return item.selectedRestaurantIds?.some((id) => form.selectedRestaurantIds.includes(id))
        }
        return false
      })
      .filter((item) => {
        ...
        return Number.isFinite(itemStart) && Number.isFinite(itemEnd) && itemStart <= end && itemEnd >= start
      })
      .slice(0, 4)
  }, [existingVouchers, form.endsAt, form.restaurantId, form.scopeType, form.selectedRestaurantIds, form.startsAt, voucher?._id])
  ...
  function toggleSelection(
    key: "categoryIds" | "itemIds" | "selectedRestaurantIds" | "selectedCustomerIds",
    id: string,
    checked: boolean
  ) {
    setForm((current) => ({
  ...
  function validate() {
    ...
    if (form.scopeType === "selected_restaurants" && form.selectedRestaurantIds.length === 0) {
      nextErrors.selectedRestaurantIds = "Select at least one restaurant."
    }
    if (form.audienceType === "selected_users" && form.selectedCustomerIds.length === 0) {
    ...
    if (!form.allowRepeatUsage && maxUsesPerUser > 1) {
      nextErrors.maxUsesPerUser = "Turn on repeat usage before allowing more than 1 use."
    }
    if (form.maxTotalUses.trim() && Number(form.maxTotalUses) < maxUsesPerUser) {
    ...
    if (form.scopeType === "restaurant" && form.applicability === "categories" && form.categoryIds.length === 0) {
      nextErrors.categoryIds = "Select at least one category."
    }
    if (form.scopeType === "restaurant" && form.applicability === "items" && form.itemIds.length === 0) {
            ...
            <div className="grid gap-5 lg:grid-cols-2">
              <div className="space-y-2 lg:col-span-2">
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="selected_restaurants">Selected restaurants</SelectItem>
                ...
                <div className="space-y-2 lg:col-span-2">
                  <TargetCheckboxList
                    ...
                    options={restaurants.map((restaurant) => ({
                      ...
                      helper: restaurant.city || restaurant.address || restaurant.ownerName,
                    }))}
              ...
              <div className="space-y-2 lg:col-span-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => update("audienceType", value as VoucherFormState["audienceType"])}
                >
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
              ...
              {form.audienceType === "selected_users" ? (
                <div className="space-y-2 lg:col-span-2">
                  <TargetCheckboxList
                    ...
                    emptyText={customersQuery.isLoading ? "Loading users..." : "No customers found."}
                    ...
                    onToggle={(id, checked) => toggleSelection("selectedCustomerIds", id, checked)}
                  />
              ...
              <div className="space-y-2">
                <Label>Code <span className="text-xs text-muted-foreground">(only for coupon)</span></Label>
                <Input
                  value={form.code}
              ...
              <div className="space-y-2">
                <Label>Mode</Label>
                <Select value={form.mode} onValueChange={(value) => update("mode", value as AdminVoucherMode)}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
              ...
              <div className="space-y-2">
                ...
                <Select value={form.type} onValueChange={(value) => update("type", value as VoucherFormType)}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
              ...
              <div className="space-y-2">
                <Label>Discount value <span className="text-xs text-muted-foreground">(optional for free delivery)</span></Label>
                <Input
                  type="number"
                  min="0"
              ...
              <div className="space-y-2">
                ...
                <Input type="number" min="0" value={form.minimumOrderAmount} onChange={(event) => update("minimumOrderAmount", event.target.value)} />
                <FieldError message={errors.minimumOrderAmount} />
              </div>

              <div className="space-y-2">
                <Label>Funding</Label>
                <Select value={form.fundedBy} onValueChange={(value) => update("fundedBy", value as VoucherFormState["fundedBy"])}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
              ...
              {form.fundedBy === "shared" ? (
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(event) => update("ownerSharePercent", event.target.value)}
                  />
              ...
              <div className="space-y-2">
                ...
                <Select value={form.stackingRule} onValueChange={(value) => update("stackingRule", value as VoucherFormState["stackingRule"])}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
              ...
              <div className="space-y-2">
                ...
                <Input type="number" min="0" value={form.priority} onChange={(event) => update("priority", event.target.value)} />
              </div>
              <div className="space-y-2">
                <Label>Status</Label>
                <Select value={form.status} onValueChange={(value) => update("status", value as AdminVoucherStatus)}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
              ...
              <div className="space-y-2">
                <Label>Max total uses <span className="text-xs text-muted-foreground">(optional)</span></Label>
                <Input type="number" min="0" value={form.maxTotalUses} onChange={(event) => update("maxTotalUses", event.target.value)} />
                <FieldError message={errors.maxTotalUses} />
              </div>
              <div className="space-y-2">
                <Label>Max uses per user</Label>
                <Input type="number" min="1" value={form.maxUsesPerUser} onChange={(event) => update("maxUsesPerUser", event.target.value)} />
              ...
              <div className="flex items-center justify-between rounded-lg border p-3 lg:col-span-2">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Turn on when one customer can redeem more than once.</p>
                </div>
                <Switch checked={form.allowRepeatUsage} onCheckedChange={(checked) => update("allowRepeatUsage", checked)} />
              </div>
                ...
                {conflictPreview.length ? (
                  <div className="grid gap-2">
                    {conflictPreview.map((item) => (
                      <div key={item._id} className="flex items-center justify-between rounded-md border bg-background px-3 py-2 text-sm">
                  ...
                  <p className="text-sm text-muted-foreground">
                    No overlapping active campaign found for this scope and date window.
                  </p>
                )}
              </div>
              ...
              <div className="space-y-3 lg:col-span-2">
                ...
                <Select value={form.applicability} onValueChange={(value) => update("applicability", value as VoucherFormState["applicability"])}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
                ...
                {form.scopeType !== "restaurant" ? (
                  <div className="rounded-lg border bg-muted/30 p-3 text-sm text-muted-foreground">
                    Platform campaigns apply to all eligible items in the selected scope.
                  </div>
                ) : null}
                {form.scopeType === "restaurant" && form.applicability === "categories" ? (
                  <div className="space-y-2">
                    <TargetCheckboxList
                      title="Categories"
                      emptyText={
                      ...
                      onToggle={(id, checked) => toggleSelection("categoryIds", id, checked)}
                    />
                    <FieldError message={errors.categoryIds} />
                  </div>
                ) : null}
                {form.scopeType === "restaurant" && form.applicability === "items" ? (
                  <div className="space-y-2">
                    <TargetCheckboxList
                      title="Menu items"
                      emptyText={
                      ...
                      onToggle={(id, checked) => toggleSelection("itemIds", id, checked)}
                    />
...
export function CustomerHomeCmsSection({
  ...
  const [draftContent, setDraftContent] = React.useState<PlatformContent | null>(content)
  ...
  React.useEffect(() => {
    ...
    setScheduledAtInput(content?.customerApp.homeCms.pushCampaign.scheduledAt?.slice(0, 16) ?? "")
  }, [content])

  const cms = draftContent?.customerApp.homeCms

  if (isLoading || !content || !draftContent || !cms) {
    return (
      <Card>
  ...
  function updateCarouselPath(index: number, ctaPath: string) {
    updateCms({
      ...currentCms,
      offerStrip: {
        ...currentCms.offerStrip,
        carouselImages: currentCms.offerStrip.carouselImages.map((image, itemIndex) =>
          itemIndex === index ? { ...image, ctaPath } : image
        ),
      },
    })
  }
  ...
  const previewButtonClass =
    previewBlock.buttonStyle === "dark"
      ? "bg-[#2B1D24] text-white"
      : previewBlock.buttonStyle === "outline"
        ? "border bg-transparent"
        : previewBlock.buttonStyle === "soft"
          ? "bg-white/65"
          : "text-white"
  ...
  return (
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
                    ...
                    >
                      ...
                      {previewBlock.variant !== "image" ? (
                        <div className="min-w-0 flex-1 space-y-1.5">
                          ...
                          <span className={`inline-flex rounded-full px-3 py-1.5 text-xs font-black ${previewButtonClass}`} style={{ borderColor: previewBlock.accentColor, backgroundColor: previewBlock.buttonStyle === "pill" ? previewBlock.accentColor : undefined, color: previewBlock.buttonStyle === "outline" ? previewBlock.accentColor : undefined }}>
                            {previewBlock.ctaLabel || "Explore"}
                          </span>
                        </div>
                      ) : null}
                    </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Offer strip slot</CardTitle>
            <CardDescription>Show voucher strip, hide it, or replace it with a CMS block.</CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4 lg:grid-cols-3">
            ...
            <div className="space-y-2">
              ...
              <Input value={cms.offerStrip.ctaPath} onChange={(event) => updateOfferStrip("ctaPath", event.target.value)} />
            </div>
            ) : null}
            ...
            <div className="space-y-2">
              ...
              <Select value={cms.offerStrip.buttonStyle} onValueChange={(value) => updateOfferStrip("buttonStyle", value as typeof cms.offerStrip.buttonStyle)}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="pill">Solid pill</SelectItem>
                  <SelectItem value="soft">Soft glass</SelectItem>
                  <SelectItem value="outline">Outline</SelectItem>
                  <SelectItem value="dark">Dark contrast</SelectItem>
                </SelectContent>
              </Select>
            </div>
            ) : null}
                ...
                {cms.offerStrip.carouselImages.map((image, index) => (
                  <div key={`${image.publicId}-${index}`} className="flex items-center justify-between gap-3 rounded-lg border p-3">
                    ...
                    <div className="flex items-center gap-2">
                      <Input className="h-8 w-56" value={image.ctaPath ?? ""} placeholder="Slide path" onChange={(event) => updateCarouselPath(index, event.target.value)} />
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Show a YouTube tutorial or step-by-step images on the customer home screen.</CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4 lg:grid-cols-3">
            <div className="flex items-center justify-between rounded-lg border p-3 lg:col-span-3">
              <div>
            ...
            <div className="space-y-2">
              ...
              <Input value={cms.howToOrderGuide.youtubeUrl} onChange={(event) => updateGuide("youtubeUrl", event.target.value)} placeholder="https://youtube.com/..." />
            </div>
            <div className="space-y-2">
              <Label>Title</Label>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Click any card to inspect recent user-level activity.</CardDescription>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Timed campaign modal on home screen.</CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4">
            ...
            <Input value={cms.modal.ctaPath} onChange={(event) => updateModal("ctaPath", event.target.value)} placeholder="CTA path" />
        ...
        {hidePushCampaign ? (
          <Card className="xl:col-span-2">
            <CardHeader>
              <CardTitle>Promotional push moved</CardTitle>
              <CardDescription>
                Push campaigns now live in Notifications so scheduling,
                recipient reports, A/B testing, and conversions stay in one
        ...
        <Card className="xl:col-span-2">
          ...
          <CardContent className="grid gap-4 lg:grid-cols-3">
            ...
            <Input value={cms.pushCampaign.path} onChange={(event) => updatePush("path", event.target.value)} placeholder="Path" />
      ...
      <SheetContent className="flex h-full w-full max-w-none! flex-col overflow-hidden p-0 sm:max-w-3xl! md:max-w-6xl!">
        <div className="border-b px-6 py-5">
          <SheetHeader>
            ...
            <SheetDescription>
              {getScopeLabel(voucher)} - {getAudienceLabel(voucher.audienceType)} - {getVoucherModeLabel(voucher.mode)} - {formatVoucherDiscount(voucher)}
            </SheetDescription>
          </SheetHeader>
          <div className="mt-4 flex flex-wrap items-center gap-2">
            <Badge variant="outline" className={getLifecycleBadgeClass(lifecycle)}>{lifecycle}</Badge>
            <Badge variant="secondary">{getVoucherTypeLabel(voucher.type)}</Badge>
            <Badge variant="outline">{getVoucherFundingLabel(voucher.fundedBy)}</Badge>
            <Badge variant="outline">Created by {voucher.createdByType}</Badge>
            <Button size="sm" className="ml-auto" onClick={() => onEdit(voucher)}>
          ...
          <div className="grid gap-4 xl:grid-cols-[0.95fr_1.05fr]">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Same rule model used in restaurant-owner promotions.</CardDescription>
              </CardHeader>
              <CardContent className="grid gap-3 text-sm">
                <InfoRow label="Scope" value={getScopeLabel(voucher)} />
                <InfoRow label="Audience" value={getAudienceLabel(voucher.audienceType)} />
                <InfoRow
                  label="Restaurant"
                  value={voucher.restaurant?.name ?? voucher.restaurantId ?? "Platform campaign"}
                />
                <InfoRow label="Code" value={voucher.code || "Auto applied"} />
                <InfoRow label="Mode" value={getVoucherModeLabel(voucher.mode)} />
                <InfoRow label="Type" value={getVoucherTypeLabel(voucher.type)} />
                <InfoRow label="Discount" value={formatVoucherDiscount(voucher)} />
                <InfoRow
                  label="Max discount cap"
                  value={voucher.maxDiscountAmount ? formatCurrency(voucher.maxDiscountAmount) : "No cap"}
                />
                <InfoRow label="Minimum order" value={formatCurrency(voucher.minimumOrderAmount)} />
                <InfoRow label="Max total uses" value={voucher.maxTotalUses ?? "Unlimited"} />
                <InfoRow label="Per user" value={voucher.allowRepeatUsage ? `${voucher.maxUsesPerUser} times` : "Once"} />
                <InfoRow label="Funding" value={getVoucherFundingLabel(voucher.fundedBy)} />
                <InfoRow
                  label="Funding split"
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Delivered-order revenue and redemption analytics.</CardDescription>
              </CardHeader>
              <CardContent className="grid gap-3 text-sm">
                <InfoRow label="Total uses" value={voucher.analytics.totalUses} />
                <InfoRow label="Unique users" value={voucher.analytics.uniqueUsers} />
                <InfoRow label="Repeat usage" value={voucher.analytics.repeatUsage} />
                <InfoRow label="Orders using voucher" value={voucher.analytics.totalOrdersUsingVoucher} />
                <InfoRow label="Revenue generated" value={formatCurrency(voucher.analytics.revenueGenerated)} />
                <InfoRow label="Discount given" value={formatCurrency(voucher.analytics.totalDiscountGiven)} />
                <InfoRow label="Delivery cost covered" value={formatCurrency(voucher.analytics.totalDeliveryCostCovered)} />
                <InfoRow label="Remaining usage" value={voucher.analytics.remainingUsage ?? "Unlimited"} />
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="grid gap-3 text-sm">
                ...
                <InfoRow label="Created" value={formatDate(voucher.createdAt)} />
                <InfoRow label="Updated" value={formatDate(voucher.updatedAt)} />
                <InfoRow label="Archived" value={voucher.archivedAt ? formatDate(voucher.archivedAt) : "No"} />
                <InfoRow label="Creator id" value={voucher.createdById || "N/A"} />
                {voucher.archiveReason ? (
                  <InfoRow label="Archive reason" value={voucher.archiveReason} />
                ) : null}
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-3 text-sm">
                ...
                {voucher.applicability === "categories" ? (
                  <div className="flex flex-wrap gap-2">
                    {(categoryNames.length ? categoryNames : voucher.categoryIds).map((name) => (
                ...
                {voucher.applicability === "items" ? (
                  <div className="flex flex-wrap gap-2">
                    {(itemNames.length ? itemNames : voucher.itemIds).map((name) => (
                      <Badge key={name} variant="outline">{name}</Badge>
                    ))}
                  </div>
                ) : null}
            ...
            <Card className="xl:col-span-2">
              <CardHeader>
                ...
                <CardDescription>Daily usage and delivered-order discount impact.</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="overflow-hidden rounded-lg border">
                  <Table>
                    ...
                    <TableBody>
                      {voucher.analytics.points.map((point) => (
                        <TableRow key={point.label}>
                          ...
                          <TableCell className="text-right">{formatCurrency(point.discount)}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
            ...
            <Card className="xl:col-span-2">
              <CardHeader>
                ...
                <CardDescription>Recent admin/owner changes for dispute visibility.</CardDescription>
              </CardHeader>
              <CardContent>
                      ...
                      <TableBody>
                        {voucher.recentAudits.map((audit) => (
                          <TableRow key={audit.id}>
                            ...
                            <TableCell>{audit.actorType} {audit.actorId}</TableCell>
                            <TableCell>{audit.note || "N/A"}</TableCell>
                            <TableCell className="text-right">{formatDate(audit.createdAt)}</TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                    </Table>
...
export function CouponsPage() {
  ...
  const [scopeType, setScopeType] = React.useState<"all" | VoucherFormState["scopeType"]>("all")
  ...
  const [selectedVoucher, setSelectedVoucher] = React.useState<AdminRestaurantVoucher | null>(null)
  const [editingVoucher, setEditingVoucher] = React.useState<AdminRestaurantVoucher | null>(null)
  ...
  const vouchersQuery = useQuery({
    queryKey: ["admin-vouchers", { search: debouncedSearch, lifecycle, mode, type, scopeType, restaurantId, sortBy, page, pageSize }],
    queryFn: () =>
      listAdminVouchers({
  ...
  const createMutation = useMutation({
    ...
    onError: (error) => toast.error(error instanceof Error ? error.message : "Failed to create offer"),
  })

  const updateMutation = useMutation({
    mutationFn: ({ voucherId, payload }: { voucherId: string; payload: AdminVoucherPayload }) =>
    ...
    onError: (error) => toast.error(error instanceof Error ? error.message : "Failed to update offer"),
  })
  ...
  const archiveMutation = useMutation({
    ...
    onError: (error) => toast.error(error instanceof Error ? error.message : "Failed to archive promotion"),
  })
  ...
  const restoreMutation = useMutation({
    ...
    onError: (error) => toast.error(error instanceof Error ? error.message : "Failed to restore promotion"),
  })
  ...
  const activeCount = vouchers.filter((voucher) => getVoucherLifecycleStatus(voucher) === "Active").length
  const deliveredRevenue = vouchers.reduce((sum, voucher) => sum + voucher.analytics.revenueGenerated, 0)
  const discountGiven = vouchers.reduce((sum, voucher) => sum + voucher.analytics.totalDiscountGiven, 0)
  const platformFunded = vouchers.filter((voucher) => voucher.fundedBy === "platform").length
  const existingCodes = vouchers.map((voucher) => voucher.code).filter(Boolean)

  React.useEffect(() => {
    setPage(1)
  }, [debouncedSearch, lifecycle, mode, type, scopeType, restaurantId, sortBy, pageSize])
  ...
  return (
    <>
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Manage platform and restaurant promotions with owner-synced rules and analytics.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard label="Active offers" value={activeCount} helper="On current page" />
        <StatCard label="Total offers" value={vouchersQuery.data?.total ?? 0} helper="All matching filters" />
        <StatCard label="Delivered revenue" value={formatCurrency(deliveredRevenue)} helper="Voucher order revenue" />
        <StatCard label="Discount given" value={formatCurrency(discountGiven)} helper={`${platformFunded} platform funded`} />
      </div>

      <Card>
        <CardHeader>
          <CardTitle>Promotion directory</CardTitle>
          <CardDescription>Same voucher analytics source used by restaurant-owner promotions.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex flex-wrap items-center gap-2">
            <div className="relative min-w-64 flex-1">
              <Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
              <Input className="h-9 pl-9" placeholder="Search offer or code" value={search} onChange={(event) => setSearch(event.target.value)} />
            </div>
            <Select value={lifecycle} onValueChange={(value) => setLifecycle(value as AdminVoucherLifecycle)}>
            ...
            <Select value={mode} onValueChange={(value) => setMode(value as "all" | AdminVoucherMode)}>
              <SelectTrigger className="h-9 w-full sm:w-36"><SelectValue /></SelectTrigger>
              <SelectContent>
            ...
            <Select value={type} onValueChange={(value) => setType(value as VoucherTypeFilter)}>
            ...
            <Select value={sortBy} onValueChange={(value) => setSortBy(value as VoucherSort)}>
              <SelectTrigger className="h-9 w-full sm:w-44"><SelectValue /></SelectTrigger>
              <SelectContent>
                  ...
                  return (
                    <TableRow key={voucher._id}>
                      <TableCell>
                        ...
                        <div className="text-xs text-muted-foreground">{voucher.code || "Auto applied"} - {formatVoucherDiscount(voucher)}</div>
                        <Badge variant="outline" className={getLifecycleBadgeClass(lifecycleStatus)}>{lifecycleStatus}</Badge>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        ...
                        <div className="text-xs text-muted-foreground">{getVoucherTypeLabel(voucher.type)}</div>
                      </TableCell>
                      <TableCell>{getVoucherFundingLabel(voucher.fundedBy)}</TableCell>
                      <TableCell>
                        <div>{voucher.analytics.totalUses} uses</div>
                        <div className="text-xs text-muted-foreground">{voucher.analytics.uniqueUsers} users</div>
                      </TableCell>
                      <TableCell>
                        <div>{formatCurrency(voucher.analytics.revenueGenerated)}</div>
                        <div className="text-xs text-muted-foreground">Discount {formatCurrency(voucher.analytics.totalDiscountGiven)}</div>
                      </TableCell>
                      <TableCell>
                        <div>{formatDate(voucher.startsAt)}</div>
                        <div className="text-xs text-muted-foreground">until {formatDate(voucher.endsAt)}</div>
                      </TableCell>
                      <TableCell>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            {voucher.archivedAt ? (
                              <DropdownMenuItem
                                ...
                                onClick={() => restoreMutation.mutate(voucher._id)}
                              >
          ...
          <div className="flex flex-col gap-3 border-t pt-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="text-sm text-muted-foreground">Showing {vouchers.length} of {vouchersQuery.data?.total ?? vouchers.length} offers</div>
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
              <Select value={`${pageSize}`} onValueChange={(value) => setPageSize(Number(value))}>
                <SelectTrigger className="w-full sm:w-32"><SelectValue /></SelectTrigger>
                <SelectContent side="top">
                  {pageSizeOptions.map((size) => <SelectItem key={size} value={`${size}`}>{size} / page</SelectItem>)}
                </SelectContent>
              </Select>
              <div className="text-sm font-medium">Page {safePage} of {pageCount}</div>
              <div className="flex items-center gap-2">
                <Button variant="outline" size="icon" disabled={safePage <= 1} onClick={() => setPage((current) => Math.max(1, current - 1))}>
                  <ChevronLeft className="size-4" />
                </Button>
                <Button variant="outline" size="icon" disabled={safePage >= pageCount} onClick={() => setPage((current) => Math.min(pageCount, current + 1))}>
                  <ChevronRight className="size-4" />
                </Button>
              </div>
            </div>
